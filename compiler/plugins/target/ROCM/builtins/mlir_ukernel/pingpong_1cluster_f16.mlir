...
//
// Tile: 128x128, K-block: 32, Warps: 4 (256 threads), Stages: 3.
// Triple-buffered LDS (3 x 128x32 per operand = 48KB total).
...
// Per K-block: 128x32 = 4096 f16 -> 8 gathers -> 2 per subgroup.

!in_ty = tensor<128x?xf16>
!in_buf_ty = memref<128x?xf16, strided<[?, 1], offset: ?>, #amdgpu.address_space<fat_raw_buffer>>
// Triple-buffered: buf 0 = rows [0,128), buf 1 = [128,256), buf 2 = [256,384).
!shared_ty = memref<384x32xf16, #gpu.address_space<workgroup>>
// Expanded: 24 M-groups x 16 lanes x 1 K-step x 32 K-elems.
!shared_exp_ty = memref<24x16x1x32xf16, #gpu.address_space<workgroup>>
...
util.func private @pingpong_1cluster_f16(
  ...
  %lhs_exp = memref.expand_shape %lhs_shared [[0, 1], [2, 3]]
      output_shape [24, 16, 1, 32] : !shared_ty into !shared_exp_ty
  ...
  scf.forall (%tid) in (256) {
    ...
    scf.for %stage = %c0 to %c2 step %c1 {
      ...
      %buf_off = arith.muli %stage, %c128 overflow<nsw, nuw> : index
  ...
  %result = pcf.generic scope(#iree_gpu.subgroup_scope)
    ...
    pcf.generic scope(#iree_gpu.lane_scope)
      ...
      %loop:2 = scf.for %k = %c64 to %dim step %c32
        ...
        // Buffer management.
        %read_off = arith.muli %read_buf, %c8 overflow<nsw, nuw> : index
        ...
        %write_off = arith.muli %write_buf, %c128 overflow<nsw, nuw> : index
      ...
      %epi_off_0 = arith.muli %loop#1, %c8 overflow<nsw, nuw> : index
      ...
      %epi_off_1 = arith.muli %epi_nb, %c8 overflow<nsw, nuw> : index
