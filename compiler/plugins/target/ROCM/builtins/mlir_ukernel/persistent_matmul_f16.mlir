// RUN: iree-opt %s
//
// Triton-inspired persistent kernel matmul ukernel for gfx950 (CDNA4/MI350).
// Uses amdgpu.gather_to_lds for async global-to-LDS DMA copies.
//
// Persistent scheduling: each workgroup processes MULTIPLE output tiles in a
// loop, reducing kernel launch overhead and improving hardware utilization
// when there are more tiles than CUs.
//
// Per-tile: 128x128 output, K-block: 64, Warps: 4 (256 threads), Stages: 2.
...
// Schedule: Single memory cluster + single dot cluster (1-cluster pingpong).
...
//
// Outer loop: scf.for iterates over N output tiles. LDS buffers are allocated
// once and reused across tile iterations. Each iteration starts fresh with a
// zero accumulator, runs the full K-loop, and writes the result back.
...
util.func private @persistent_matmul_f16(
    %lhs_base: !in_ty, %rhs_base: !in_ty,
    %unused_acc: tensor<128x128xf32>,
    %num_tiles: index) -> tensor<128x128xf32> {
  ...
  // Double-buffered LDS for LHS and RHS, shared across ALL tile iterations.
  ...
  // Each iteration processes one 128x128 output tile using the full 1-cluster
  // pingpong schedule. The output tensor is threaded through as an iter_arg so
  // each tile's result accumulates into the final output.
  // In a real persistent kernel, %lhs_base/%rhs_base would be offset by
  // %tile_idx to select different input tiles. For this demonstration, we
  // reuse the same inputs for each iteration.
  %final = scf.for %tile_idx = %c0 to %num_tiles step %c1
      iter_args(%out_acc = %unused_acc) -> (tensor<128x128xf32>) {

    // =================================================================
    // PROLOGUE: DMA first K-block (k=0) into buffer 0 (rows [0,128)).
    // Must reload each tile iteration because LDS is reused and the
    // previous tile's epilogue left stale data.
    // =================================================================
    ...
    // =================================================================
    // PER-TILE COMPUTATION: pcf.generic with pingpong K-loop.
    // =================================================================
    %tile_result = pcf.generic scope(#iree_gpu.subgroup_scope)
      execute(%out_ref = %out_acc)[%sg_id: index, %num_sg: index]
    ...
    // Barrier between tile iterations: ensure all threads have finished
    // writing before the next tile's prologue overwrites LDS.
    gpu.barrier memfence [#gpu.address_space<workgroup>]

    scf.yield %tile_result : tensor<128x128xf32>
  }
  util.return %final : tensor<128x128xf32>
}
