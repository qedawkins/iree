...
static ConvolutionDimType getConvolutionDimType(
    unsigned dim, mlir::linalg::detail::ConvolutionDimensions convDims) {
  if (llvm::is_contained(convDims.batch, dim)) return ConvolutionDimType::Batch;
  ...
  if (llvm::is_contained(convDims.depth, dim)) return ConvolutionDimType::Depth;
...
static DenseMap<unsigned, ConvolutionDimType> getConvolutionDimMap(
  ...
  for (unsigned b : convDims.batch) convDimMap[b] = ConvolutionDimType::Batch;
  ...
  for (unsigned d : convDims.depth) convDimMap[d] = ConvolutionDimType::Depth;
  return convDimMap;
}
...
rewriteInIm2ColGeneric(RewriterBase & rewriter, linalg::GenericOp genericOp) {
  ...
  auto findDimPosFromIndex = [](int64_t dimPos, int64_t index,
    ...
    for (int i = index, e = dimOrder.size(); i < e; i++) {
      if (dimOrder[i] == dimPos) return i;
    }
    return static_cast<int>(dimOrder.size());
  };

  auto updateReassociationAndShapeVecs =
      [&](int64_t dim, int64_t prevIndex, int64_t nextIndex,
          bool prevCollapsible, SmallVector<int64_t> dimOrder,
  ...
  for (auto dim : im2ColDimOrder) {
    if (isa<ConvolutionDimType::InputChannel, ConvolutionDimType::FilterLoop>(
      ...
      updateReassociationAndShapeVecs(dim, filterDimPos, nextFilter,
                                      prevWasFilterCollapsible, filterDimOrder,
                                      filterShape, filterReassocIndices);
      ...
      updateReassociationAndShapeVecs(dim, outputDimPos, nextOutput,
                                      prevWasOutputCollapsible, outputDimOrder,
                                      outputShape, outputReassocIndices);
  ...
  auto img2ColTensor = rewriter.create<linalg::GenericOp>(
      ...
      [&](OpBuilder &nestedBuilder, Location nestedLoc, ValueRange args) {
        ...
        for (auto [index, group] : llvm::enumerate(im2ColReassocIndices)) {
          ...
          auto unrolledIndices =
              unrollIndex(nestedBuilder, nestedLoc, iterator, groupShape);
          for (auto [dim, unrolledIndex] :
               llvm::zip_equal(group, unrolledIndices))
        ...
        for (auto [imageDim, filterDim, expr, stride] :
             llvm::zip_equal(dimensions.outputImage, dimensions.filterLoop,
                             convolvedExprs, dimensions.strides)) {
          assert(
              expr.isFunctionOfDim(imageDim) &&
              expr.isFunctionOfDim(filterDim) &&
              "convolved indices mismatch with indexing order in source map");
          convolvedIndices.push_back(
              getConvolvedIndex(nestedBuilder, nestedLoc, indexMap[imageDim],
                                indexMap[filterDim], stride));
        }
  ...
  auto advanceOperandIndex =
      [&](int &l, int r, SmallVector<ReassociationIndices> groupedDimOrder) {
        ...
        for (; l < r; l++) {
          ...
          if (leadingGroupDimToIterationDim.count(groupDim)) continue;
  ...
  while (inputIndex < inputEnd && filterIndex < filterEnd &&
    ...
    ConvolutionDimType lhsDimType =
        filterRHS ? convDimMap[im2ColReassocIndices[inputIndex][0]]
                  : convDimMap[groupedFilterDimOrder[filterIndex][0]];
    bool isLastLhsDim = filterRHS
                            ? im2ColReassocIndices.size() - 1 == inputIndex
    ...
    if (groupedFilterDimOrder[filterIndex][0] ==
        im2ColReassocIndices[inputIndex][0]) {
  ...
  auto newGenericOp = rewriter.create<linalg::GenericOp>(
      ...
      /*inputs=*/
          filterRHS ? ValueRange{img2ColTensor.getResult(0), reshapedFilter}
