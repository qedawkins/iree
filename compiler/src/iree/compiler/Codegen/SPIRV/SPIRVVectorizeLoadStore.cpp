...
struct ScalarizeVectorTransferRead final
  ...
  LogicalResult matchAndRewrite(vector::TransferReadOp readOp,
    ...
    if (vectorType.getRank() == 0) {
      Value maybeMaskBit;
      if (maybeMask) {
        maybeMaskBit = rewriter.create<vector::ExtractOp>(loc, maybeMask, ArrayRef<int64_t>{0});
      }
    ...
    for (int i = 0; i < vectorType.getDimSize(0); ++i) {
      ...
      if (maybeMask) {
        ...
        // vector.
        maybeMaskBit = rewriter.create<vector::ExtractOp>(loc, maybeMask, ArrayRef<int64_t>{i});
      }
