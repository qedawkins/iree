...
static bool adjustToPromote(ArrayRef<int64_t> dimMNKSize, int64_t &mTileSize,
  ...
  auto usedBytes = getTileBytes(mTileSize, nTileSize, kTileSize, elementBits, false);
...
LogicalResult setCooperativeMatrixConfig(
  ...
  auto storeStage = softwarePipelineStoreStage;
  const int maxBytes = limits.getMaxComputeSharedMemorySize();
  auto usedBytes = getTileBytes(workgroupTileSizes[mIndex], workgroupTileSizes[nIndex],
          reductionTileSizes[kIndex], getElementType(lhs).getIntOrFloatBitWidth(), promoteC);
  ...
  while (pipelineDepth > 0 && getMultiBufferMemoryUsage(usedBytes, pipelineDepth, storeStage) >
        maxBytes) {
    pipelineDepth--;
  }
