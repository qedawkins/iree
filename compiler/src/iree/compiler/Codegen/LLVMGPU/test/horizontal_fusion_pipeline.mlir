// RUN: iree-opt --iree-gpu-test-target=gfx942 --pass-pipeline="builtin.module(iree-llvmgpu-select-lowering-strategy, func.func(iree-codegen-lower-dispatch-using-pipeline-attr))" %s --split-input-file | FileCheck %s --check-prefixes=CHECK,CDNA3
// RUN: iree-opt --iree-gpu-test-target=gfx950 --pass-pipeline="builtin.module(iree-llvmgpu-select-lowering-strategy, func.func(iree-codegen-lower-dispatch-using-pipeline-attr))" %s --split-input-file | FileCheck %s --check-prefixes=CHECK,CDNA4
