// RUN: iree-opt --pass-pipeline='builtin.module(func.func(iree-codegen-lower-dispatch-using-pipeline-attr))' %s | FileCheck %s
// RUN: iree-opt --pass-pipeline='builtin.module(func.func(iree-codegen-lower-dispatch-using-pipeline-attr))' %s | FileCheck %s

// Verify that a custom pass pipeline specified via #iree_codegen.pass_pipeline
// attribute is executed by the LLVMGPU and ROCDL lower executable target passes.
