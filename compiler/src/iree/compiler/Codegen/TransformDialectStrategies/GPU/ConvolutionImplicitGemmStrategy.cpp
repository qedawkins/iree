...
void mlir::iree_compiler::gpu::ConvolutionImplicitGemmStrategy::configure(
  ...
  if (captures.convolutionDims.inputChannel.size() == 1 &&
    ...
    reductionLoopTileSizes = SmallVector<int64_t>(captures.convolutionDims.batch.size() + 2, 0);
    ...
    if (isNchw)
      im2ColThreadTileSizes.push_back(0);
    im2ColThreadTileSizes.push_back(numThreadsXForIm2Col);

    elementwiseThreadTileSizes = SmallVector<int64_t>(captures.convolutionDims.batch.size(), 0);
    if (!tileM)
      elementwiseThreadTileSizes.push_back(0);
    elementwiseThreadTileSizes.push_back(numThreadsXToDistribute);

    matmulWarpTileSizes = SmallVector<int64_t>(captures.convolutionDims.batch.size(), 0);
    if (!tileM)
    ...
    int channelSize = 1;
    int imageSize = captures.convolutionOpSizes[captures.convolutionDims.outputImage.back()];
    for (auto dim : captures.convolutionDims.outputChannel)
      channelSize *= captures.convolutionOpSizes[dim];
    ...
    for (auto dim : captures.convolutionDims.filterLoop)
      kSize *= captures.convolutionOpSizes[dim];
    if (captures.convolutionDims.inputChannel.size() > 1) {
      for (int i = 0, e = captures.convolutionDims.inputChannel.size() - 1; i < e; i++)
        kSize *= captures.convolutionOpSizes[captures.convolutionDims.inputChannel[i]];
    } else {
      kSize *= captures.convolutionOpSizes[captures.convolutionDims.inputChannel.front()];
    }
    ...
    im2ColThreadTileSizes = SmallVector<int64_t>(captures.convolutionDims.batch.size()
           + captures.convolutionDims.inputChannel.size() - 1, 0);
    im2ColThreadTileSizes.push_back(numThreadsXForIm2Col);

    elementwiseThreadTileSizes = SmallVector<int64_t>(captures.convolutionDims.batch.size()
           + captures.convolutionDims.outputChannel.size() - 1, 0);
    elementwiseThreadTileSizes.push_back(numThreadsXToDistribute);
...
void mlir::iree_compiler::gpu::buildConvolutionImplicitGemmStrategy(
  ...
  iree_compiler::buildTileFuseDistToForallWithNumThreads(
      ...
      /*opsHToFuse=*/{},
      /*numThreads=*/getAsOpFoldResult(b.getI64ArrayAttr(strategy.getElementwiseThreadTileSizes())),
      /*threadDimMapping=*/b.getArrayAttr({strategy.allThreadAttrs.front()}));
  ...
  iree_compiler::buildTileFuseDistToForallWithNumThreads(
      ...
      /*opsHToFuse=*/{},
      /*numWarps=*/getAsOpFoldResult(b.getI64ArrayAttr(strategy.getMatmulWarpTileSizes())),
