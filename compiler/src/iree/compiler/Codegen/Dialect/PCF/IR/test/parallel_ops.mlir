...
util.func private @generic(%0: tensor<?xi32>, %1: tensor<?x?xi32>, %d0: index, %d1: index, %n: index) {
  %2:4 = pcf.generic scope(#pcf.dummy_scope) count(%n)
    ...
    util.optimization_barrier %num_threads, %ref, %ref_1, %ref_2, %ref_3 : index, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>
    pcf.return
  } {hello = "world"}
...
//  CHECK-NEXT:     initialize(%[[REF:[A-Za-z0-9_]+]] = %[[ARG0]],
...
util.func private @generic_memref(%0: memref<?xi32>, %1: memref<?x?xi32>, %d0: index, %d1: index, %n: index) {
  %2:4 = pcf.generic scope(#pcf.dummy_scope) count(%n)
    initialize(%ref = %0, %ref_1, %ref_2, %ref_3 = %1)[%num_threads: index]
            : (!pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>)
