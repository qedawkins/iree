...
/// Forward declaration of addReadonlyAndReadwriteArgs from
/// DistributedFuseConsumers.cpp. This is defined in the anonymous namespace
/// there, so we re-declare the explicit specializations we need here.
/// Instead, we use the public helper pattern: create a new scoped op with
/// additional sref args. We replicate the helper here since it lives in
/// DistributedFuseConsumers.cpp's anonymous namespace.

/// Creates a new scoped op with additional readonly sref args. Moves the body
/// from the old op to the new one, inserts new block args, and replaces the
...
LoopOp addReadonlyArgs<LoopOp>(RewriterBase &rewriter, LoopOp loopOp,
  ...
  // Keep existing readwrite inits, result types, is_tied, dynamic sizes.
  ...
  // Update the num_readonly_refs property.
