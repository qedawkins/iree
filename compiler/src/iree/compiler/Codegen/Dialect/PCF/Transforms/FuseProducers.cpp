...
/// corresponding sref region argument that provably read the init value (i.e.,
/// are not dominated by any pcf.write_slice to the same sref). This is the
/// mirror of lookupProducerSlices in FuseConsumers.cpp.
...
lookupConsumerSlices(OpResult result,
  ...
  // Collect writes and reads on this sref arg separately.
  SmallVector<PCF::WriteSliceOp> writes;
  SmallVector<PCF::ReadSliceOp> reads;
  for (Operation *user : tiedArg.getUsers()) {
    if (auto writeOp = dyn_cast<PCF::WriteSliceOp>(user)) {
  ...
  // If the sref has SyncOnReturn semantics, all reads see the init value
  ...
  // Without SyncOnReturn, only collect reads not dominated by any write.
  // A read sees the init value if no write to the same sref comes before it.
  DominanceInfo domInfo(owner);
  for (PCF::ReadSliceOp readOp : reads) {
    bool dominatedByWrite = llvm::any_of(writes, [&](PCF::WriteSliceOp w) {
      return domInfo.properlyDominates(w.getOperation(), readOp.getOperation());
    });
    if (!dominatedByWrite) {
      slices.push_back(readOp);
    }
  }
  return success();
}
...
template <typename OpTy>
static LogicalResult
matchTilableProducerImpl(RewriterBase &rewriter, OpTy scopedOp,
                         ProducerFusionParams &params) {
...
static void fuseTilableProducerImpl(RewriterBase &rewriter, OpTy scopedOp,
  ...
  for (PCF::ReadSliceOp readSlice : params.readSlices) {
    ...
    FailureOr<TilingResult> tiledResult =
        producer.generateResultTileValue(rewriter, /*resultNumber=*/0, offsets,
                                         sizes);
    ...
    if (auto vectorType = dyn_cast<VectorType>(readSlice.getType())) {
      ...
      replacement = vector::TransferReadOp::create(rewriter, loc, vectorType,
                                                   replacement, indices,
                                                   padding);
    }
