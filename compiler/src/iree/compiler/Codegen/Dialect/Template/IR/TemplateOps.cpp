...
void CallOp::print(OpAsmPrinter &printer) {
  ...
  printer << "<";
  const llvm::SmallVector<llvm::SmallVector<Type>> &bindings = getProperties().type_bindings;
  llvm::interleaveComma(bindings, printer, [&](const llvm::SmallVector<Type> &binding) {
    if (binding.size() == 1) {
      ...
      printer << "[";
      llvm::interleaveComma(binding, printer, [&](Type t) { printer << t; });
      printer << "]";
    }
  });
  printer << ">";
