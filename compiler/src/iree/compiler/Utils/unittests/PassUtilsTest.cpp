...
TEST(MultiOpNestTest, NamedVariableOrdering) {
  // Named MultiOpNest variable with braces -- the pattern that caused the
  // pass ordering bug. The destructor must fire before "after" is added.
  MLIRContext context;
  context.loadDialect<func::FuncDialect>();
  context.disableMultithreading();
  OwningOpRef<ModuleOp> module = createModule(context, {"alpha"});
