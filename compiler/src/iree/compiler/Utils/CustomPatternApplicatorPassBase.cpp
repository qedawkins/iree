    ...
    if (auto tensorType = dyn_cast<RankedTensorType>(val.getType())) {
      for (int64_t i = 0; i < tensorType.getRank(); ++i) {
        flatTensorSizes.push_back(
            rewriter.create<tensor::DimOp>(val.getLoc(), val, i));
      }
    }
  }
  return ValueRange(flatTensorSizes);
}

static FailureOr<ValueRange> getI32TensorSizes(PatternRewriter &rewriter,
                                               ValueRange vals) {
