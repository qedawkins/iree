...
private:
  ...
  void initializeOperation(Operation *op, DFX::Solver &solver) override {
    ...
    if (auto load = dyn_cast<IREE::Util::GlobalLoadOpInterface>(op)) {
      ...
      if (!callableOp) {
        ...
        indicateOptimisticFixpoint();
      } else if (!cast<mlir::CallableOpInterface>(callableOp)
                      .getCallableRegion()) {
  ...
  // The only operation kinds we need to update are
...
public:
  explicit GlobalAccessorAnalysis(Operation *rootOp)
    ...
    // TODO: Use op traits or something to ignore the contents of executables
    // and execution regions (they don't access globals). i.e.
  ...
  LogicalResult run() {
    ...
    if (succeeded(result)) {
      explorer.walk([&](Operation *op) {
        ...
        GlobalAccessor *maybeAccessor = solver.lookupElementFor<GlobalAccessor>(
            Position::forOperation(op),
            /*queryingElement=*/nullptr,
            /*resolution=*/DFX::Resolution::OPTIONAL,
            /*allowInvalidState=*/true);
        if (!maybeAccessor) {
          return WalkResult::advance();
        }
        if (maybeAccessor->isValidState()) {
          auto accessorMap = maybeAccessor->getAssumedMultiSet();
