...
class Convert1x1FilterConvGenericToMatmul : public OpRewritePattern<linalg::GenericOp> {
 public:
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp genericOp,
    ...
    if (!llvm::all_of(
        dimensions.filterLoop, [&](int64_t index) { return dimSizes[index] == 1; }))
      return failure();
    ...
    auto updateReassociationAndShapeVecs = [&](int64_t dim, int64_t im2ColIndex, int64_t prevIndex,
    ...
    for (auto [im2ColIndex, dim] : llvm::enumerate(im2ColDimOrder)) {
      if (isa<ConvolutionDimType::InputChannel,
        ...
        updateReassociationAndShapeVecs(dim, im2ColIndex, filterDimPos, nextFilter,
        ...
        updateReassociationAndShapeVecs(dim, im2ColIndex, outputDimPos, nextOutput,
    ...
    while (inputIndex < inputEnd &&
      ...
      ConvolutionDimType lhsDimType = filterRHS ? convDimMap[groupedIm2ColDimOrder[inputIndex][0]]
                                                : convDimMap[groupedFilterDimOrder[filterIndex][0]];
      bool isLastLhsDim = filterRHS ? groupedIm2ColDimOrder.size() - 1 == inputIndex
      ...
      if (groupedFilterDimOrder[filterIndex][0] == groupedIm2ColDimOrder[inputIndex][0]) {
      ...
      } else if (groupedOutputDimOrder[outputIndex][0] == groupedIm2ColDimOrder[inputIndex][0]) {
    ...
    auto newGenericOp = rewriter.create<linalg::GenericOp>(
        ...
        /*inputs=*/filterRHS ? ValueRange{reshapedInput, reshapedFilter}
